<template>
	<view class="position_relative width100" :style="classObj.transition + 'background-color:#ffffff;'">
		<swiper :autoplay="autoplay" :interval="interval" @change="swiperChange" @animationfinish="animationfinish"
			:circular="circular" :vertical="vertical" :previous-margin="previous_margin" :next-margin="next_margin"
			:duration="time" :display-multiple-items="display_multiple_items"
			:skip-hidden-item-layout="skip_hidden_item_layout" class="swiperStyle" :style="'height:' + swiperHeight + ';'">
			
			<swiper-item v-for="(item, indexs) in swiperItems" :key="indexs">
				<QSImage :imageUrl="item.image" imageMode="scaleToFill" @click="goToAdvertByUrl(item)" v-if="current==indexs"
					radius="0px" />
			</swiper-item>
		</swiper>
<!-- 		<view class="position_absolute" :style="classObj.indicator_view" v-if="indicator_custom">
			<view
				:style="classObj.transition + classObj.indicator_default_dots + (current==index?classObj.indicator_select_dots:classObj.indicator_unselect_dots)"
...
<script>
	...
	export default {
		...
		methods: {
			...
			goToAdvertByUrl(item) {
				this.$emit("advClick", item);
			},
		},
		onReady() {
			this.onReady = true;
