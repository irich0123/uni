<template>
	<view class="container no-scroll">
		<!-- #ifndef  H5-->
		<my-nav-bar title="交易详情" @action="navAction" @reSize="reSize" />
		<!-- #endif -->
		...
		<view class="bg-gray-1 gray-border-top flex align-center"
			...
			<view class="flex flex-grow align-center justify-end padding-left-sm  padding-lr-ssm text-black">
				...
				<view class="button_other" v-if="transaction.status === 0"
					@click.stop="goToConfirmTransaction(transaction.id)">确认订单
				</view>
				<view class="button_other" v-if="transaction.status === 1"
					@click.stop="promptPaymentTransaction(transaction.id,transaction.sellerId)">付款提醒
				</view>
				<view class="button_other" v-if="transaction.status === 2"
					@click.stop="deliveryGoodsTransaction(transaction.id,transaction.carrierId)">发货
				</view>
				<view class="button_other" v-if="transaction.status === 3"
					@click.stop="promptConfirmDeliveryTransaction(transaction.id,transaction.sellerId)">收货提醒
				</view>
				<view class="button_other"
					v-if="transaction.status === 7 || (transaction.status === 8 && transaction.overTimeStatus === transaction.status) || (transaction.status === 9 && transaction.overTimeStatus === transaction.status) || transaction.status === 14 || (transaction.status === 15 && transaction.overTimeStatus === transaction.status) || transaction.status === 17"
					@click.stop="goToDispute(transaction.id,transaction.overTimeStatus,transaction.status)">争议
				</view>
				...
				<view class="button_other" v-if="transaction.status === 9"
					@click.stop="receiveGoodsConfirmTransaction(transaction.id,transaction.sellerId)">确认收货
				</view>
				<view class="button_other"
					v-if="transaction.status === 3 || transaction.status === 4 || transaction.status === 7 || transaction.status === 8 || transaction.status === 9 || transaction.status === 10 || transaction.status === 12 || transaction.status === 14 || transaction.status === 15 || transaction.status === 17"
					@click.stop="getMappedDeliveryByTransactionId(transaction.id,0)">物流
				</view>
				<view class="button_other"
					v-if="transaction.status === 9 || transaction.status === 10 || transaction.status === 17"
					@click.stop="getMappedDeliveryByTransactionId(transaction.id,1)">退货物流
				</view>
				<view class="button_other" v-if="transaction.status === 12"
					@click.stop="getReparationByTransactionId(transaction.id,transaction.sellerId,transaction.buyerId)">
					赔偿审核
				</view>
				...
				<view class="button_other"
					v-if="transaction.status === 6 || transaction.status === 11 || transaction.status === 18"
					@click.stop="deleteTransaction(transaction.id)">删除
				</view>
			</view>
		</view>
		...
		<uni-popup ref="popupReparation" type="center" :animation="true">
			<view class="bg-white radius text-df">
				...
				<view v-if="reparationData">
					...
					<view class="text-xs text-gray-5 bg-light-yellow padding-tb-ssm padding-lr">
						赔偿金额将在订单结算时自动结转到卖家余额！
					</view>
				</view>
				<view class="flex align-center">
					<view
						class="flex-sub bg-white text-df flex align-center justify-center gray-border-right text-black"
						style="height:100rpx;border-bottom-left-radius: 20rpx;"
						@click="reparationConfirmTransaction(reparationData.transactionId,false)">
						不同意
					</view>
					<view
						class="flex-sub bg-white text-df flex align-center justify-center gray-border-right text-black"
						style="height:100rpx;" @click="$refs.popupReparation.close()">再想想</view>
					<view
						class="flex-sub bg-white text-df flex align-center justify-center gray-border-right text-theme"
						style="height:100rpx;border-bottom-right-radius: 20rpx;"
...
<script>
	...
	export default {
		name: "transactionDetailsForSeller",
		components: {
			uniPopup,
			AddressListCell,
			CommodityListCell,
		...
		methods: {
			...
			goToDispute(id) {
				uni.navigateTo({
					url: './dispute?id=' + id + '&initiatorType=1'
				})
			},
			//收货提醒 --- 卖家催买家收货
