<template>
	<view class="container no-scroll">
		<!-- #ifndef  H5-->
		<my-nav-bar title="接单交易" @action="navAction" @reSize="reSize" class="my-nav-bar" />
		<!-- #endif -->
					...
					<view class="flex-grow">
						<work-transaction-list-cell :transaction="item">
							<view class="padding-top-sm margin-top-sm gray-border-top flex align-center justify-end">
								...
								<view
									class="margin-left-ssm padding-lr padding-tb-ssm radius-xl gray-border-1 text-black text-sm"
									v-else-if="item.status === 5" @click.stop="checkDelayApply(item.id)">
									延期原因
								</view>
								<view v-else class="flex align-center justify-end text-black">
									...
									<view
										class="margin-left-ssm padding-lr padding-tb-ssm radius-xl gray-border-1 text-sm"
										v-if="item.statusCode === '110'" @click.stop="doPush(item.id,1)">催交保证金
									</view>

									<view
										class="margin-left-ssm padding-lr padding-tb-ssm radius-xl gray-border-1 text-sm"
										v-if="item.statusCode === '201'" @click.stop="doPush(item.id,7)">催发样品
									</view>
									<view
										class="margin-left-ssm padding-lr padding-tb-ssm radius-xl gray-border-1 text-sm"
										v-if="item.statusCode === '221'" @click.stop="unqualified(item.id,false,0)">不合格
									</view>
									<view
										class="margin-left-ssm padding-lr padding-tb-ssm radius-xl gray-border-1 text-sm"
										v-if="item.statusCode === '221'" @click.stop="unqualified(item.id,true,0)">合格
									</view>

									<view
										class="margin-left-ssm padding-lr padding-tb-ssm radius-xl gray-border-1 text-sm"
										v-if="item.statusCode === '210'" @click.stop="deliverGoods(item.id,0)">样品发货
									</view>
									<view
										class="margin-left-ssm padding-lr padding-tb-ssm radius-xl gray-border-1 text-sm"
										v-if="item.statusCode === '212'" @click.stop="doPush(item.id,2)">催收样品
									</view>

									<view
										class="margin-left-ssm padding-lr padding-tb-ssm radius-xl gray-border-1 text-sm"
										v-if="item.statusCode === '301'" @click.stop="doPush(item.id,9)">催发材料
									</view>
									<!--                                        材料合格不合格-->
									<view
										class="margin-left-ssm padding-lr padding-tb-ssm radius-xl gray-border-1 text-sm"
										v-if="item.statusCode === '321'" @click.stop="confirmReceiveMaterial(item.id)">
										收到材料
									</view>
									<view
										class="margin-left-ssm padding-lr padding-tb-ssm radius-xl gray-border-1 text-sm"
										v-if="item.statusCode === '401'" @click.stop="">催验收
									</view>
									...
									<view
										class="margin-left-ssm padding-lr padding-tb-ssm radius-xl gray-border-1 text-sm"
										v-if="item.statusCode === '410'" @click.stop="completedProduct(item.id)">成品完成
									</view>
									<view
										class="margin-left-ssm padding-lr padding-tb-ssm radius-xl gray-border-1 text-sm"
										v-if="item.statusCode === '411'" @click.stop="deliverGoods(item.id,1)">成品发货
									</view>
									<view
										class="margin-left-ssm padding-lr padding-tb-ssm radius-xl gray-border-1 text-sm"
										v-if="item.statusCode === '412'" @click.stop="doPush(item.id,4)">催收成品
									</view>

									<view
										class="margin-left-ssm padding-lr padding-tb-ssm radius-xl gray-border-1 text-sm"
										v-if="item.statusCode === '501'" @click.stop="doPush(item.id,5)">催付款
									</view>
									<view
										class="margin-left-ssm padding-lr padding-tb-ssm radius-xl gray-border-1 text-sm"
										v-if="item.statusCode === '510'" @click.stop="openUploadPayVoucher(item.id,2)">
										收款凭证
									</view>

									<view
										class="margin-left-ssm padding-lr padding-tb-ssm radius-xl gray-border-1 text-sm"
										v-if="item.statusCode === '600' || item.statusCode === '610'"
										@click.stop="goToEvaluate(item.id,item.secondPartUserId)">评价
									</view>
		...
		methods: {
			...
			loadUserData(param) {
				...
				getUserAccountInfoByUserId({
					...
					if (res.retCode === 0) {
						self.totalMoney = res.data.deposit / 1000 //总的保证金
				...
				uni.showModal({
					...
					success: function(res1) {
						if (res1.confirm) {
							...
							completedProduct(paramsData).then(res => {
								if (res.retCode === 0) {
									uni.showToast({
										icon: 'none',
										title: '操作成功',
					...
					if (res.retCode === 0) {
						...
						res.data.list.forEach(item => {
							if (new RegExp('甲方').test(item.statusDesc)) {
								item.statusDesc = item.statusDesc.replace('甲方', '对方')
							} else if (new RegExp('乙方').test(item.statusDesc)) {
								item.statusDesc = item.statusDesc.replace('乙方', '我方')
							}
