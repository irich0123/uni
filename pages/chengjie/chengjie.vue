	...
	<view class="container">
		...
		<view class="bg-theme position-fixed" style="top:0;left:0;right:0;z-index: 999;">
			...
			<view class="flex align-center" :style="'width:' +menubarLeftPosition+'px;height:40px'">
				<!-- #ifdef MP-WEIXIN -->
				<view class="flex-twice text-lg color-white padding-left-lg">找工厂</view>
				<!-- #endif -->
				<!-- #ifndef MP-WEIXIN -->
				<view class="flex-sub"></view>
				<view class="flex-twice text-center text-lg color-white">找工厂</view>
				<!-- #endif -->
				<!-- #ifdef APP-PLUS -->
		...
		<scroll-view class="bg-gray-1" v-if="list.length>0" scroll-y
			:style="'padding-top:'+ (contentTop +40) +'px;height:'+listHeight+'px;'"
			...
			<!--列表开始-->
			<view class="margin-top-ssm" @click.stop="showOrderDetail(item)"
...
<script>
	...
	export default {
		...
		onShow() {
			...
			if (!!uni.getStorageSync("needRefresh") || !!uni.getStorageSync("workOrderList_refresh")) {
				uni.removeStorageSync("needRefresh");
				uni.removeStorageSync("workOrderList_refresh");
				this.initData();
			}
		},
		methods: {
			...
			toSearch() {
				uni.navigateTo({
					url: '/pagesNew/dowork/searchWorkOrder'
				})
			},
		},
