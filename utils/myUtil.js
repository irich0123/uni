...
//HUD
//成功提示
function wxShowSuccess(title = "成功啦", duration = 2500) {
	uni.showToast({
		title: title,
		icon: 'success',
...
//loading提示
function wxShowLoading(title = "请稍后", duration = 5000) {
	uni.showToast({
		title: title,
		icon: 'loading',
...
//隐藏提示框
function wxHideToast() {
	uni.hideToast();
}

//显示带取消按钮的消息提示框
function wxAlertViewWithCancel(title = "提示", content = "消息提示", confirm, showCancel = "true") {
	uni.showModal({
		title: title,
		content: content,
...
function priceTranslate(number) {
	if (!number) {
		return 0;
	}
...
//科学计数法
function calculateMath(number, type) {
	if (!number) {
		return 0;
	}
	if (type === 2) { //2位小数
		if (number / 10000.00 >= 1 && number / 100000.00 < 1) {
			...
			return Number(number).toFixed(2) + "万";
			...
			return Number(number).toFixed(2) + "亿";
			...
			return Number(number).toFixed(2) + "兆";
		...
		if (number / 10000.00 >= 1 && number / 100000.00 < 1) {
			number = number / 10000.00;
			return Number(number).toFixed(0) + "万";
		} else if (number / 100000.00 >= 1 && number / 1000000.00 < 1) {
			number = number / 10000.00;
			return Number(number).toFixed(0) + "万";
		} else if (number / 1000000.00 >= 1 && number / 10000000.00 < 1) {
			...
			number = number / 100000000.00;
			return Number(number).toFixed(0) + "亿";
		} else if (number / 1000000000.00 >= 1 && number / 10000000000.00 < 1) {
			number = number / 100000000.00;
			return Number(number).toFixed(0) + "亿";
		} else if (number / 10000000000.00 >= 1 && number / 100000000000.00 < 1) {
			...
			number = number / 1000000000000.00;
			return Number(number).toFixed(0) + "兆";
		} else if (number / 10000000000000.00 >= 1 && number / 100000000000000.00 < 1) {
			number = number / 1000000000000.00;
			return Number(number).toFixed(0) + "兆";
		} else if (number / 100000000000000.00 >= 1 && number / 1000000000000000.00 < 1) {
...
function toDate(number) {
	...
	let minte = 60;
	let hour = 60 * 60;
	let just = new Date().getTime(); //获取当前时间，单位ms
...
function parseShortAddress(whereShow, address) {
...
function shortCity(str) {
	let proArr = ["兴安盟", "锡林郭勒", "阿拉善", "延边", "大兴安岭", "恩施", "湘西", "阿坝", "甘孜", "凉山", "铜仁", "黔西南", "毕节", "黔东南", "黔南",
		"楚雄", "红河", "文山", "西双版纳", "大理", "德宏", "怒江", "迪庆", "昌都", "山南", "日喀则", "那曲", "阿里", "林芝", "临夏", "甘南", "海东",
		"海北", "黄南", "海南", "果洛", "玉树", "海西", "吐鲁番", "哈密", "昌吉", "博尔塔拉", "巴音郭楞", "阿克苏", "克孜勒", "喀什", "和田", "伊犁", "塔城",
		"阿勒泰"
	];
	for (let i = 0; i < proArr.length; i++) {
		let tem = proArr[i];
...
function shortArea(str) {
	...
	let proArr = ["青龙", "丰宁", "宽城", "围场", "孟村", "大厂", "托克托", "和林格尔", "巴林左旗", "巴林右旗", "翁牛特旗", "喀喇沁旗", "敖汉旗", "库伦旗",
		"奈曼旗", "扎鲁特旗", "达拉特旗", "准格尔旗", "鄂托克前旗", "鄂托克旗", "杭棉旗", "乌审旗", "伊金霍洛旗", "阿荣旗", "陈巴尔虎旗", "乌拉特前旗", "乌拉特中旗",
		"乌拉特后旗", "杭棉后旗", "四子王旗", "扎赉特旗", "阿巴嘎旗", "太仆寺旗", "镶黄旗", "正镶白旗", "正蓝旗", "额济纳旗", "阿勒泰", "岫岩", "新宾", "清原",
		"本溪", "恒仁", "宽甸", "阜新", "伊通", "长白", "前郭", "杜尔伯特", "景宁", "长阳", "五峰", "神农架", "城步", "江华", "麻阳", "新晃", "芷江",
		"靖州", "通道", "乳源", "连山", "连南", "融水", "三江", "龙胜", "恭城", "隆林", "富川", "罗城", "环江", "巴马", "都安", "大化", "金秀", "白沙",
		"昌江", "乐东", "陵水", "保亭", "琼中", "西沙", "南沙", "中沙", "石柱", "秀山", "酉阳", "彭水", "北川", "峨边", "马边", "木里", "道真", "务川",
		"镇宁", "关岭", "紫云", "玉屏", "印江", "沿河", "松桃", "威宁", "三都", "石林", "禄劝", "寻甸", "峨山", "新平", "元江", "玉龙", "宁蒗", "普洱",
		"墨江", "景东", "景谷", "镇沅", "江城", "孟连", "澜沧", "西盟", "双江", "耿马", "沧源", "屏边", "金平", "河口", "漾濞", "南涧", "巍山", "贡山",
		"兰坪", "维西", "张家川", "天祝", "肃南", "肃北", "阿克塞", "东乡", "积石山", "大通", "民和", "互助", "化隆", "循化", "门源", "河南", "巴里坤",
		"木垒", "焉耆", "察布查尔", "和布"
	];
	for (let i = 0; i < proArr.length; i++) {
		let tem = proArr[i];
 ...
 * 删除的地区
 */
function deleteAreaList(str) {
	let flag = false;
	let areaListArr = ["密云县", "延庆县", "蓟县", "静海县", "宁河县", "藁城市", "鹿泉市", "栾城县", "唐海县", "抚宁县", "永年县", "肥乡县", "万全县", "崇礼县",
		"普兰店市", "大洼县", "双城市", "崇明县", "溧水县", "高淳县", "铜山县", "金坛市", "吴江市", "赣榆县", "洪泽县", "大丰市", "临安市", "奉化市", "洞头县",
		"上虞市", "建阳市", "永定县", "新建县", "九江县", "赣县", "东乡县", "章丘市", "垦利县", "陵县", "定陶县", "许昌县", "郧县", "电白县", "梅县", "武鸣县",
		"柳江县", "临桂县", "宜州市", "开县", "梁平县", "铜梁县", "武隆县", "潼南县", "荣昌县", "璧山县", "双流县", "郫县", "罗江县", "安县", "彭山县", "达县",
		"名山县", "遵义县", "毕节市", "铜仁市", "沾益县", "江川县", "日喀则市", "昌都县", "林芝县", "乃东县", "户县", "华县", "安塞县", "南郑县", "平安县"
	];
...
function parseActionConsume(action, userVerifyStatus, withCard) {
	let result = null;
	let list = uni.getStorageSync("ActionConsume");

	if (list != null && list.length>0) {
		list.forEach(consume => {
			if (!consume.cardStatus) {
				consume.cardStatus = 0;
			}
			if (consume.action.name===action && consume.userStatus === userVerifyStatus && consume.cardStatus === (withCard?1:0)) {
				result = consume.goldBeanExpense;
			}
		});
	}
	return result;
