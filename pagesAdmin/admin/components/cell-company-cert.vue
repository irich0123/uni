<template>
	<view class="container">
		<view class="padding-tb-xs radius-l">
			...
			<view class="flex justify-start padding-lr gray-border-bottom">
				<image class="flex-sub card-image radius-s" mode="widthFix" :src="cert.businessLicenseImg"
					@tap.stop='gotoPreview(cert,0)' />
				<image class="flex-sub margin-left card-image radius-s" mode="widthFix" :src="cert.gateImg"
					@tap.stop='gotoPreview(cert,1)' />
				<image class="flex-sub margin-left card-image radius-s" mode="widthFix" :src="cert.officeImg"
	...
	export default {
		...
		methods: {
			...
			gotoPreview(item, index) {
				uni.previewImage({
					current: index,
					urls: [item.businessLicenseImg,item.gateImg,item.officeImg],
					loop: true,
				});
			},
		}
