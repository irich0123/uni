<template>
	<view class="main no-scroll">
		<!-- #ifndef  H5-->
		<my-nav-bar title="承接信息审核" @action="navAction" @reSize="reSize" class="my-nav-bar" />
		<!-- #endif -->
		...
		<scroll-view class="bg-gray-1" scroll-y :style="'height:'+listHeight+'px;'" @scrolltolower="loadMoreData"
			...
			<view v-for="(item,idx) in tabBars[tabIndex].list" :key="idx" class="margin-lr-sm">
				<view class="bg-white margin-top radius-x" @click="toDetail(item)">
					<work-order-list-cell :work-order="item" :show-admin-bar="true" :show-footer="false">
...
<script>
	...
	export default {
		...
		methods: {
			...
			confirmOkModal() {
				this.auditOk(this.curWorkOrder);
			},
			confirmFailModal() {
				...
				this.auditFail(this.curWorkOrder);
			},
			loadMoreData() {
			...
			loadData(index) {
				...
				filterWorkOrder(param).then(res => {
					if (res.retCode === 0) {
			...
			toDetail(item) {
				uni.navigateTo({
					url: '/pagesNew/details/workDetails?fromAdmin=1&id=' + item.id
				})
			},
		}
	}
