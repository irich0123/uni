		...
		<scroll-view class="bg-gray-1" scroll-y :style="'height:'+listHeight+'px;'" @scrolltolower="loadMoreData"
			...
			<view v-for="(item,idx) in tabBars[tabIndex].list" :key="idx" class="margin-lr-sm">
				<view class="bg-white margin-top radius-x">
					<cell-company-cert :cert="item" :certStatus="certStatusList" @action="actionHandle" />
				</view>
			</view>
...
<script>
	...
	import cellCompanyCert from '@/pagesAdmin/admin/components/cell-company-cert.vue';
	...
	import {
		filterUserComAuthInfo,
		filterAuditRemark,
		auditUserComAuthInfoOk,
		auditUserComAuthInfoFail
	} from '@/api/admin'
	import {
	...
	export default {
		...
		methods: {
			...
			loadData() {
				...
				filterUserComAuthInfo(param).then(res => {
					if (res.retCode === 0) {
